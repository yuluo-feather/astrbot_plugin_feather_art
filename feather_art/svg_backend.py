"""SVG 后端（可选方言）：同一份中间表示，写成内联 SVG 的单文件 HTML。
...
d 属性按增量写：顶点密的地方走相对（子路径首点相对上一条子路径的起点、其余
...
def _path_data(regions) -> str:
    ...
    （首点相对上一条子路径的起点、其余顶点走隐式相对 lineto，m 之后的坐标对
    ...
    parts = []
    previous = (0, 0)                                  # 上一条子路径的起点（子路径以 Z 闭合，这才是 Z 之后的当前点）
