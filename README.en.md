...
<p align="center">
  "Hand me an image, get back something that opens on HTML + CSS alone. Hmph — no internet needed." — Yuluo
</p>
...
This is an AstrBot plugin I wrote myself: it **traces an image offline** into a single-file illustration — by default **pure HTML + CSS**: no `<img>`, no SVG, no Canvas, no JavaScript, no base64, no external assets. (There's an optional inline-SVG dialect too — still a single file, still zero scripts and zero external links; see the `render_backend` config key.) Opening the result needs nothing but a browser. Animated GIF / WebP inputs become **pure-CSS looping animations**: I track layers frame to frame, then write a @keyframes timeline the browser plays on its own.

## 🎨 Features

- 🖼️ **Offline tracing**: bitmap → single-file HTML; I compute everything locally, with zero API dependency
...
- 🎨 **Perceptual quantization**: Oklab median-cut in perceptual space; I fit local gradients inside large regions and lay a 48-color underpainting underneath — less banding, less detail loss
- 🎚️ **Four presets**: sketch / motion / freehand (default) / finebrush; when `--fit` overshoots I step down **colors first, width last**, so finer presets keep their resolution
- 🛡️ **Contract audit**: I run my own red lines over every artifact (no scripts / images / external links / base64 injection), plus an offline MAE similarity score
...
Four presets, and freehand is the one I keep on day to day — enough for the job, easy on the machine. Max width is the widest it will trace; anything wider gets scaled down to it before drawing starts.
...
| finebrush · 工笔 | 3600 | 256 | 0.24 | 4 | detail first; larger & slower |
...
| Cap the size | `/羽画 --fit N` | target volume (MiB) with 2% tolerance; over budget it steps down colors first, then width |
| Trace an animation | send a GIF / WebP | automatically uses the motion preset → looping pure-CSS animation; very long ones (>30 s or 2500+ frames) are prompted for first, and tracing starts once you say 继续 |
...
From the moment you send an image to the moment I hand over the result, this is the pipeline I walk — understand it and troubleshooting gets much easier.
...
User request (either entry)[main.py entry orchestration]
   ...
   │
   ▼
(1) Grab & cache the image[main.py]
   ├─ The first image is taken from the message (the framework has already downloaded it locally)
   └─ Each person's latest image is kept for 30 minutes — LLM-tool flows and the "继续" (continue) confirmation never need the original resent
   │
   ▼
(2) Parse options[options.py]
   └─ preset / --fit / --sample are recognized in any order ("/羽画 图片 工笔" is the same as "/羽画 工笔 图片")
   │
   ▼
(3) Rate-limit gate[limiter.py]
   ...
   │
   ▼
(4) Input hardening[hardening.py]
   ├─ Size cap (max_image_mb, default 64 MiB)
   ├─ Decompression-bomb guard (max_pixels, default 40M)
   ├─ Animations: frame count read + duration estimate; over-long ones (>30 s or 2500 frames) are prompted first, "继续" admits them
   └─ Failed checks get human-friendly words; internal exceptions go to logs only
   │
   ▼
(5) Tracing orchestration
   ...
   │     └─ --fit N: over budget the ladder jumps straight from a size estimate (colors first, width last), with no per-step full retries; the budget is judged on the **actual bytes produced**, so the same target usually steps down one rung less on the svg dialect
   ...
   │
   ▼
(6) Contract audit & scoring[feather_art/audit.py · score.py]
   ...
   │
   ▼
(7) Delivery[deliver.py]
   ├─ Summary text (human words, no raw paths) + a single-file HTML (readable filename: 羽画_工笔_0908-2214.html)
   └─ Failure layering: explainable errors get human words; internal exceptions go to logger and return a fixed fallback line
```
...
Still images go through eight steps:
...
8. Single-file HTML generation, then the contract audit runs over it
...
1. Clone or download this repo into AstrBot's `data/plugins/` directory, keeping the folder name `astrbot_plugin_feather_art` (the package name the plugin expects):
...
2. Install dependencies (needed for tracing; not needed to view the generated HTML):
...
- Quality: the sketch and freehand presets trace wider too — max width 768 → 1152 and 1200 → 1600, so small images and everyday illustrations are no longer shrunk ahead of time (measured: error down 8–16% and 7–11% at the original size)
...
- Fix: internal failure text no longer reaches you — phrasing such as "file already exists: xxx.html" stays internal, and text written for users is no longer mistaken for an internal fault
...
- Performance: faster budget stepping — no more retrying every ladder step with a full render; size estimates jump straight to a fitting step, noticeably cutting the wait for heavy images
...
- Photos, noise and dense textures come out as large files; use `--fit` or a lighter preset
...
- Very long animations (>30 s or 2500+ frames) are prompted for first: use the original GIF or trim a 10–15 s clip, and tracing runs only after you confirm
...
- The static tracing pipeline (quantization / merging / contours / gradients) takes its algorithm ideas from [AvroraCL/image-to-css-art](https://github.com/AvroraCL/image-to-css-art) (MIT) — the implementation is mine, rewritten, no code copied
- The *animated GIF / WebP → pure-CSS animation* idea came from [kevinjycui/css-video](https://github.com/kevinjycui/css-video) — I replaced its per-frame clustering + index alignment with frame-to-frame layer tracking (IoU + color + centroid matching); an independent design of mine, no code copied
- [javierbyte/img2css](https://github.com/javierbyte/img2css) sits on my desk as a classic reference (per-pixel box-shadow path), one I never took
