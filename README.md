...
| --- | ---: | ---: | ---: | ---: | --- |
| 速写 | 1152 | 96 | 0.38 | 2 | 小图、表情包、印章，快而轻 |
| 动画 | 512 | 96 | 0.36 | 2 | 动图专用：多帧描摹，快而稳 |
| 写意（默认） | 1600 | 160 | 0.30 | 3 | 日常插画，均衡之选 |
| 工笔 | 2000 | 256 | 0.24 | 4 | 细节优先，输出更大更慢 |
...
从你发图/开口到拿到成品，羽画在内部走这样一条流水线——理解它，出问题就好排查了。
> 括号里的模块名是各环节的实现归属（详见[开发](#开发)）：流程怎么走看这里，逻辑在哪看模块名。

```
用户请求（双入口任选其一）【main.py 入口编排】
   ├─ 命令入口：「/羽画 [档位] [--fit N] [--sample N]」+ 发图
   │     （私聊直接说即可，群聊 @ 机器人；档位与图片顺序随意）
   └─ 自然语言入口：发图后说「把这张图做成纯 CSS 插画」（llm_tool，可关）
   │
   ▼
① 取图与缓存【main.py】
   ├─ 从消息里取第一张图（框架下载到本地）
   └─ 该用户 30 分钟内的最近一张图自动保留——LLM 工具场景与动图「继续」确认都不必重发原图
   │
   ▼
② 解析指令【options.py】
   └─ 档位 / --fit / --sample 随写随认（「/羽画 图片 工笔」与「/羽画 工笔 图片」等价）
   │
   ▼
③ 限流闸门【limiter.py】
   ├─ 全局并发（concurrent，默认 1）：同时只跑一个，其余排队
   └─ 每用户冷却（cooldown，默认 60s）：冷却期内拦下，提示还需等多久
   │
   ▼
④ 输入防护【hardening.py】
   ├─ 体积上限（max_image_mb，默认 20 MiB）
   ├─ 像素炸弹防线（max_pixels，默认 4000 万）
   ├─ 动图：读帧数并估算总时长；超长动图（>30 秒或 2500 帧）先提示后描摹，回复「继续」才放行
   └─ 检查不过的错误只回人话；内部异常只进日志
   │
   ▼
⑤ 描摹编排
   ...
   └─ 动图【service_animation.py trace_animation】
         ...
         └─ 渲染风格（animation_style）：scanline（默认，扫描线，无碎块无缝）｜ vector（旧管线，支持补间 tween）
   │
   ▼
⑥ 契约审计与评分【feather_art/audit.py · score.py】
   ...
   ▼
⑦ 交付【deliver.py】
   ├─ 摘要文案（纯人话，不含原始路径）+ HTML 单文件（文件名带可读时间戳：羽画_工笔_0908-2214.html）
   └─ 失败分层：能解释的错误回人话；内部异常只进 logger，回固定兜底文案
```
...
| `fit_mb` | `64` | 输出体积目标（MiB），含 2% 容差；`0` = 不自动降档 |
| `max_mb` | `128` | 输出体积硬上限（MiB） |
...
| `render_backend` | `css` | 静态描摹的渲染后端：css 纯 CSS 插画（默认，产品身份）/ svg 内联 SVG（可选方言：体积约省一半；同一份成品在 Chromium / Firefox / WebKit 三个内核准逐位一致——实测样本为单张图、三档宽度；动图不受影响） |
| `concurrent` | `1` | 同时进行的描摹任务数 |
| `cooldown` | `60` | 同一用户相邻描摹冷却秒数 |
| `llm_tool` | `true` | 自然语言入口开关 |
| `max_image_mb` | `64` | 输入图片体积上限 |
...
- 修复：群里别人发的图以前会被描成说话人自己的旧图——连着两次求画收到的是同一张；现在引用消息里的图会被取出来，且取图只取请求者自己发的那张，自己没发过就直说「先发图」
...
- 优化：动图样式体积略降，同样的画面文件更小

#### v0.3.0

- 新增：可选的内联 SVG 方言——配置项 `render_backend` 选 `svg`，同一张图描成内联 SVG 单文件，体积约为纯 CSS 的一半；默认仍是纯 CSS
- 新增：交付文件名可读化，`羽画_工笔_0908-2214.html` 一眼看懂是哪一版
- 修复：v0.2.2 装上后所有入口误报「缺少依赖」、动图采样参数直接报错——本版补齐
- 修复：离线相似度评分不再虚报
